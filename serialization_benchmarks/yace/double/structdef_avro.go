...
func (a *AvroA) Unmarshal(d []byte, o interface{}) error {
	...
	rec := i.(*goavro.Record)
	temp, _ := rec.Get("name")
	object.Name = temp.(float64)
	temp, _ = rec.Get("birthday")
	object.BirthDay = temp.(float64)
	temp, _ = rec.Get("phone")
	object.Phone = temp.(float64)
	temp, _ = rec.Get("siblings")
	object.Siblings = temp.(float64)
	temp, _ = rec.Get("spouse")
	object.Spouse = temp.(float64)
...
func avroUnmarshal(d []byte, o interface{}, unmarshalFunc func([]byte) (interface{}, []byte, error)) error {
	...
	object.Name = m["name"].(float64)
	object.BirthDay = m["birthday"].(float64)
	object.Phone = m["phone"].(float64)
	object.Siblings = m["siblings"].(float64)
	object.Spouse = m["spouse"].(float64)
