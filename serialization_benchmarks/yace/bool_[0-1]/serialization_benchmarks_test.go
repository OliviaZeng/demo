...
func generate() []*A {
	...
	for i := 0; i < 1000; i++ {
		a = append(a, &A{
			Name:     rand.Intn(2) == 1,
			BirthDay: rand.Intn(2) == 1,
			Phone:    rand.Intn(2) == 1,
			Siblings: rand.Intn(2) == 1,
...
func (s *FlatBufferSerializer) Unmarshal(d []byte, i interface{}) error {
	...
	a.Name = o.Name() == byte(1)
	a.BirthDay = o.BirthDay() == byte(1)
	a.Phone = o.Phone() == byte(1)
	a.Siblings = o.Siblings() == byte(1)
	a.Spouse = o.Spouse() == byte(1)
	a.Money = o.Money() == byte(1)
...
func (s *HproseSerializer) Marshal(o interface{}) []byte {
	...
	l := buf.Len()
	writer.WriteBool(a.Name)
	writer.WriteBool(a.BirthDay)
	writer.WriteBool(a.Phone)
	writer.WriteBool(a.Siblings)
	writer.WriteBool(a.Spouse)
	writer.WriteBool(a.Money)
	return buf.Bytes()[l:]
}
...
func (s *HproseSerializer) Unmarshal(d []byte, i interface{}) error {
	...
	o.Name, _ = reader.ReadBool()
	o.BirthDay, _ = reader.ReadBool()
	o.Phone, _ = reader.ReadBool()
	o.Siblings, _ = reader.ReadBool()
	o.Spouse, _ = reader.ReadBool()
	o.Money, _ = reader.ReadBool()
	return nil
}
...
func generateProto() []*ProtoBufA {
	...
	for i := 0; i < 1000; i++ {
		a = append(a, &ProtoBufA{
			Name:     proto.Bool(rand.Intn(2) == 1),
			BirthDay: proto.Bool(rand.Intn(2) == 1),
			Phone:    proto.Bool(rand.Intn(2) == 1),
			Siblings: proto.Bool(rand.Intn(2) == 1),
			Spouse:   proto.Bool(rand.Intn(2) == 1),
			Money:    proto.Bool(rand.Intn(2) == 1),
		})
	}
	return a
}
